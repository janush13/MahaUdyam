...
describe('Auth e2e — RBAC, permissions and department scoping', () => {
  ...
  describe('roles (RolesGuard)', () => {
    ...
    it('authorises from the database, not from the JWT roles claim', async () => {
      ...
      // The DB is authoritative, so the guard allows it — proving the claim
      // is not what decides. (MFA is enforced at LOGIN; this pre-existing
      // 15-minute access token was issued before the role existed.)
      await get('officer-only', accessToken).expect(200);
      // ...and the reverse: a forged claim cannot help, covered in
      // auth-session (tampered payload / wrong secret => 401).
    });
  });
