...
export class UsersService {
  ...
  /** Current roles + MFA flag in one query, for MfaEnforcementGuard —
   * returns null when the user no longer exists. */
  async getMfaEnforcementState(
    userId: string,
  ): Promise<{ mfaEnabled: boolean; roleCodes: string[] } | null> {
    ...
    return {
      ...
      roleCodes: user.userRoles.map((row) => row.role.code),
    };
  }
